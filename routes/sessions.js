...
router.get('/stats', authenticate, (req, res) => {
  ...
  if (req.user.role === 'trainer') {
    total = db.prepare('SELECT COUNT(*) as cnt FROM sessions WHERE trainer_id = ?').get(req.user.id).cnt;
    upcoming = db.prepare("SELECT COUNT(*) as cnt FROM sessions WHERE trainer_id = ? AND date >= date('now')").get(req.user.id).cnt;
    ...
    total = db.prepare('SELECT COUNT(*) as cnt FROM sessions').get().cnt;
    upcoming = db.prepare("SELECT COUNT(*) as cnt FROM sessions WHERE date >= date('now')").get().cnt;
    totalBookings = db.prepare("SELECT COUNT(*) as cnt FROM bookings WHERE status = 'confirmed'").get().cnt;
  }
...
router.get('/:id', authenticate, (req, res) => {
  const session = db.prepare(SESSION_SELECT + ' WHERE s.id = ?').get(req.params.id);
...
router.post('/', requireRole('trainer', 'admin'), (req, res) => {
  ...
  try {
    ...
    const session = db.prepare(SESSION_SELECT + ' WHERE s.id = ?').get(result.lastInsertRowid);
...
router.put('/:id', authenticate, (req, res) => {
  ...
  const updated = db.prepare(SESSION_SELECT + ' WHERE s.id = ?').get(req.params.id);
...
router.delete('/:id', authenticate, (req, res) => {
  ...
  db.prepare('DELETE FROM bookings WHERE session_id = ?').run(req.params.id);
