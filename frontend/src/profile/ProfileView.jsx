...
function ProfileView() {
  ...
  const submitUpdate = useCallback(() => {
    ...
    fetchAPI("PUT", "/users/" + user.id, { name, phone, address }, localStorage.getItem("auth-key"))
      .then(response => {
        if (response.status == 200) { setStatus("Profile updated successfully!"); refresh(); }
        else { setStatus("Update failed: " + response.body.message); }
        setLoading(false);
      })
      .catch(error => { setStatus(String(error)); setLoading(false); });
  }, [name, phone, address, user, refresh]);

  if (!user) return <span className="loading loading-spinner loading-xl m-4"></span>;
  ...
  return (
    <section className="flex flex-col gap-4 p-4 items-center">
      ...
      <fieldset className="fieldset rounded-box border p-4 self-stretch">
        ...
        <input value={name} onChange={e => setName(e.target.value)} className="input w-full" type="text" />
        {validationErrors.name && <label className="label text-red-500">{validationErrors.name}</label>}

        <label className="label">Phone</label>
        <input value={phone} onChange={e => setPhone(e.target.value)} className="input w-full" type="tel" />
        {validationErrors.phone && <label className="label text-red-500">{validationErrors.phone}</label>}

        <label className="label">Address</label>
        <input value={address} onChange={e => setAddress(e.target.value)} className="input w-full" type="text" />
      </fieldset>

      {status && (
        <span className={status.includes("success") ? "text-success self-start" : "text-error self-start"}>
          {status}
        </span>
      )}

      <button disabled={loading} onClick={submitUpdate} className="btn btn-primary btn-xl self-stretch">
        Update Profile
        {loading && <span className="loading loading-spinner loading-sm"></span>}
      </button>

      {user.role === "trainer" && (
        <XMLDownloadButton route="/xml/sessions"
          authenticationKey={localStorage.getItem("auth-key") || ""}
          filename="my-sessions.xml" className="btn btn-outline self-stretch">
      ...
      {user.role === "member" && (
        <XMLDownloadButton route="/bookings/xml"
          authenticationKey={localStorage.getItem("auth-key") || ""}
          filename="my-bookings.xml" className="btn btn-outline self-stretch">
