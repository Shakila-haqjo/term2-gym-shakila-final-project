...
export class DatabaseModel {
  ...
  // Returns [rows, fields] — compatible with [[row]] = await X.execute(...) destructuring
  ...
  static async initialize() {
    ...
    try {
      // Run CREATE TABLE IF NOT EXISTS from database.sql
      ...
      await conn.execute(insertUser, ['Admin User',    'admin@gym.com', hash('admin123'),   '555-0100', '1 Gym Plaza',    'admin',   'active', ts]);
      const [r2] = await conn.execute(insertUser, ['Sarah Connorddd', 'sarah@gym.com', hash('trainer123'), '555-0101', '2 Fitness Ave',  'trainer', 'active', ts]);
      const [r3] = await conn.execute(insertUser, ['Mike Johnson',    'mike@gym.com',  hash('trainer123'), '555-0102', '3 Health Blvd',  'trainer', 'active', ts]);
      const [r4] = await conn.execute(insertUser, ['Alice Smith',     'alice@gym.com', hash('member123'),  '555-0201', '10 Oak Street',  'member',  'active', ts]);
      const [r5] = await conn.execute(insertUser, ['Bob Brown',       'bob@gym.com',   hash('member123'),  '555-0202', '11 Pine Lane',   'member',  'active', ts]);
      const [r6] = await conn.execute(insertUser, ['Carol White',     'carol@gym.com', hash('member123'),  '555-0203', '12 Maple Drive', 'member',  'active', ts]);
      ...
      await conn.execute(insertBlog, ['Top 10 Tips for Beginner Gym-Goers', trainer1Id, 'Fitness Tips',
...
<h2>5. Rest and Recovery</h2><p>Your muscles grow during rest, not during workouts. Make sure to get enough sleep.</p>`,
      ...
      await conn.execute(insertBlog, ['The Science of Muscle Building', trainer2Id, 'Science',
...
<h2>Protein Synthesis</h2><p>Adequate protein intake (0.8-1g per pound of bodyweight) supports muscle repair and growth.</p>`,
      ...
      await conn.execute(insertBlog, ['Yoga for Stress Relief', trainer1Id, 'Wellness',
...
<h2>Physical Release</h2><p>Gentle stretching releases physical tension stored in muscles due to stress.</p>`,
      ...
      await conn.execute(insertBlog, ['Nutrition for Peak Performance', trainer2Id, 'Nutrition',
